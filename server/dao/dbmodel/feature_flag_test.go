...
func TestFromModelFeatureFlag(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: dbmodel2.FeatureFlag{
				...
				Name:        "my-flag",
				Description: testutils.String("my flag description"),
				Variations: dbmodel2.JSONB(map[string]interface{}{
					"A": "a",
					"B": "b",
				}),
				Type:         model.FlagTypeBoolean,
				BucketingKey: testutils.String("teamID"),
				Metadata: dbmodel2.JSONB(map[string]interface{}{
					"key": "value",
				}),
				TrackEvents:     testutils.Bool(true),
				Disable:         testutils.Bool(false),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := dbmodel2.FromModelFeatureFlag(tt.mff)
...
func TestToModelFeatureFlag(t *testing.T) {
	...
	tests := []struct {
		name    string
		dbFF    dbmodel2.FeatureFlag
		dbRule  []dbmodel2.Rule
			...
			dbRule: []dbmodel2.Rule{
				{
					...
					Disable:       false,
					Percentages:   testutils.JSONB(dbmodel2.JSONB(map[string]interface{}{"A": float64(50), "B": float64(50)})),
					IsDefault:     false,
